   ...
   public function importepipe()
    {
     ...
     Excel::load('estimated\linelist.xlsx', function($reader) {
     ...
     foreach ($reader->get() as $epipe) {
        ...
        $units_id = DB::select("SELECT id FROM units WHERE name="."'".$epipe->units_id."'"); 
        $diameters_id = DB::select("SELECT id FROM diameters WHERE dn=".$epipe->diameter); 
        ...
        $units_count = DB::select("SELECT COUNT(id) AS units_count FROM units WHERE name="."'".$epipe->units_id."'"); 
        $diameters_count = DB::select("SELECT COUNT(id) diameters_count FROM diameters WHERE dn=".$epipe->diameter); 
    ...
    //if (($units_count[0]->units_count != 0) AND ($diameters_count[0]->diameters_count != 0) AND ($fluids_count[0]->fluids_count != 0) AND ($specs_count[0]->specs_count != 0)){ // OJO NO BORRAR + PARA LISTA DE LÍNEAS COMPLETA***************
        ...
        if (($diameters_count[0]->diameters_count != 0)){
     ...
     'source' => $epipe->source,
         ...
         echo "Error in line: ".$flag." unit: ".$units_count[0]->units_count." diameter: ".$diameters_count[0]->diameters_count." fluid: ".$fluids_count[0]->fluids_count." spec: ".$specs_count[0]->specs_count."<br>";
