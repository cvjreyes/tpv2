    ...
    $(function () {
        $('.add').click(function () {
            ...
            var tr = '<tr class="fila">' +
                    ...
                    '<td>{!! Form::select('units_id[]', [null => 'Select Area...'] + $units, null, array( 'style'=>'height:31px','rinstred')) !!}</td>'+
                    '<td>{!! Form::number('est_qty[]', null, array('placeholder' => 'Quantity','class' => 'form-control','rinstred')) !!}</td>'+
...
<table id="milestoneinst" class="table table-hover table-condensed" style="width:100%">
    <center><thead>
        <tr>
            <th>Date</th>
            <th>Area</th>
...
$(document).ready(function() {
    oTable = $('#milestoneinst').DataTable({
        ...
        "processing": true,
        "serverSide": false,
        "ajax": "{{ route('instpment.milestone') }}",
        "columns": [
            {data: 'date', name: 'date'},
            {data: 'area', name: 'area'},
...
@extends('instpment.createmilestoneinst')
<br>
<br>
<br>
...
<br>
  ...
  <button onclick="location.href='{{ url('glineinst') }}'" type="button" class="btn btn-primary btn-lg">LineChart</button>
