...
<!doctype html>
<script type="text/javascript">
   $(document).on('click', '.comments-iso-modal', function() {
  ...
  $(document).on('click', '.reject-iso-modal', function() {
  ...
  $(document).on('click', '.upload-design-modal', function() {
   ...
   $(document).on('click', '.comments-isobulk-modal', function() {
...
<div class="container">
  <div class="row">
    ...
    <center><h2 style="padding-top: 4%"><b>Iso Controller</b></h2>
      <h3>LDE/Isocontrol</h3></center>
      <div class="panel-body">
        <form id="frm-example" class="form-horizontal" role="form" method="POST" action="{{ url('/sendfromisobulk') }}">    
       {!! csrf_field() !!}     
...
$(document).on('click', '.show-vcomments-modal', function() {
    ...
    <link href="https://cdn.datatables.net/buttons/1.4.2/css/buttons.dataTables.min.css" rel="stylesheet">
      ...
      <tbody>
            <?php
              $filename = scandir("../public/storage/isoctrl/iso");
              $num=0;
    ...
    <tr>
        ...
        $pdfcl= "../public/storage/isoctrl/iso/attach/".$afilename[0]."-CL.pdf";
        $bfile= "../public/storage/isoctrl/iso/attach/".$afilename[0].".bfile";
        $dxf= "../public/storage/isoctrl/iso/attach/".$afilename[0].".dxf";
        $cii= "../public/storage/isoctrl/iso/attach/".$afilename[0].".cii";
      ...
      <td>
        ...
        <?php if (auth()->user()->hasRole('IsoctrlAdmin')){ ?> 
          ...
          <?php if ($requested[0]->requested!=1 AND $requested[0]->requestedlead!=1){ ?> <!-- Si no tiene solicitud desde diseño o Lead puede enviar a siguiente etapa -->

               <a class="comments-iso-modal btn btn-xs btn-success" data-filename ="<?php echo $filename[$i]; ?>" data-requestbydesign ="<?php echo $requested[0]->requested; ?>" data-requestbylead ="<?php echo $requested[0]->requestedlead; ?>" data-toggle="modal" data-target="#commentsfromisoModal">To Issue</a>

          <?php } ?>

            <a class="reject-iso-modal btn btn-xs btn-danger" data-filename ="<?php echo $filename[$i]; ?>" data-requestbydesign ="<?php echo $requested[0]->requested; ?>" data-requestbylead ="<?php echo $requested[0]->requestedlead; ?>" data-toggle="modal" data-target="#rejectfromisoModal">With Comments</a>
          ...
          <?php }else{ ?>
            ...
            <a class="comments-iso-modal btn btn-xs btn-success" data-filename ="<?php echo $filename[$i]; ?>" data-toggle="modal" data-target=""><b><?php echo $issued[0]->to; ?></b></a>
          ...
          <?php  if ($requested[0]->issued!=2){ ?> 
            ...
            <?php if ($requested[0]->requestedlead==1){ ?>  <!-- Switch para enviar o cancelar solicitud -->

                 <a href="reqfromlead/<?php echo $filename[$i]; ?>/0" class="btn btn-xs btn-danger" data-filename ="<?php echo $filename[$i]; ?>" data-requestlead = "0">Cancel Request</a>

            <?php }else{ ?>
         
                  <a href="reqfromlead/<?php echo $filename[$i]; ?>/1" class="btn btn-xs btn-warning" data-filename ="<?php echo $filename[$i]; ?>" data-requestlead = "1">Request</a>
        ...
        <?php }else{ ?>
          ...
          <a class="comments-iso-modal btn btn-xs btn-info" data-filename ="<?php echo $filename[$i]; ?>" data-toggle="modal" data-target=""><b>NO ACTIONS AVAILABLE!</b></a>
    ...
    <!-- BUTTONS FOR SELECT ALL  --> 
      ...
      {!! Form::select('trn[]', [null => 'Select Transmittal...'] + $trn , null, array('style'=>'height:34px;font-size: 14px;font-weight: normal;')) !!}

      <button class="btn btn-sm btn-success" name="destination" value="toissue">To Issue</button>
   ...
   <center>
        ...
        <button onclick="location.href='{{ url('iso') }}'" type="button" class="btn btn-primary btn-lg" style="width:15%"><b>LDE/Isocontrol</b></button>
