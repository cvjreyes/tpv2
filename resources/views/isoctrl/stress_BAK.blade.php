...
<p>  
      ...
      <td>
       ...
       <?php if (auth()->user()->hasRole('StressAdmin')){ ?> 
           ...
           <?php if ($requested[0]->requested==0){ ?> <!-- Si no tiene solicitud desde diseño puede enviar a siguiente etapa -->
              ...
              <a class="comments-stress-modal btn btn-xs btn-success" data-filename ="<?php echo $filename[$i]; ?>" data-toggle="modal" data-target="#commentsfromstressModal">Send to Supports</a>


          <?php } ?>

          <a class="reject-stress-modal btn btn-xs btn-danger" data-filename ="<?php echo $filename[$i]; ?>" data-toggle="modal" data-target="#rejectfromstressModal">With Comments</a>
          <a class="upload-design-modal btn btn-xs btn-info" data-filename ="<?php echo $filename[$i]; ?>" data-pathfrom="stress" data-toggle="modal" data-target="#uploadfromdesignModal">Upload File</a>
    ...
    <center>
        ...
        <button onclick="location.href='{{ url('stress') }}'" type="button" class="btn btn-primary btn-lg">Stress</button>
        <img src="{{ asset('images/arrow-icon.png') }}" style="width:30px" >
