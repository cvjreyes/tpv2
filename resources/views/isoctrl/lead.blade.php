...
<div class="container">
  <div class="row">
    ...
    <center><h2 style="padding-top: 4%"><b>Iso Controller</b></h2>
      <h3>LDG/Isochecker</h3></center>
      <div class="panel-body">
         <form id="frm-example" class="form-horizontal" role="form" method="POST" action="{{ url('/sendfromleadbulk') }}">    
       {!! csrf_field() !!}         
...
$(document).on('click', '.show-vcomments-modal', function() {
    ...
    <tr>
        ...
        $cii= "../public/storage/isoctrl/lead/attach/".$afilename[0].".cii";
      ...
      <td>
           ...
           <?php if ($requested[0]->requested!=1 AND $requested[0]->requestedlead!=1){ ?> <!-- Si no tiene solicitud desde diseño puede enviar a siguiente etapa -->
               ...
               <a class="comments-lead-modal btn btn-xs btn-success" data-filename ="<?php echo $filename[$i]; ?>" data-requestbydesign ="<?php echo $requested[0]->requested; ?>" data-requestbylead ="<?php echo $requested[0]->requestedlead; ?>" data-toggle="modal" data-target="#commentsfromleadModal">Send to Isoctrl</a>

          <?php } ?>

          <a class="reject-lead-modal btn btn-xs btn-danger" data-filename ="<?php echo $filename[$i]; ?>" data-requestbydesign ="<?php echo $requested[0]->requested; ?>" data-requestbylead ="<?php echo $requested[0]->requestedlead; ?>" data-toggle="modal" data-target="#rejectfromleadModal">With Comments</a>
          <a class="upload-design-modal btn btn-xs btn-info" data-filename ="<?php echo $filename[$i]; ?>" data-pathfrom="lead"  data-requestbydesign ="<?php echo $requested[0]->requested; ?>" data-requestbylead ="<?php echo $requested[0]->requestedlead; ?>" data-toggle="modal" data-target="#uploadfromdesignModal">Upload File</a>
     ...
     <!-- BUTTONS FOR SELECT ALL  --> 
      ...
      <button class="btn btn-sm btn-success" name="destination" value="isoctrl">Send to IsoCtrl</button>
   ...
   <center>
        ...
        <button onclick="location.href='{{ url('lead') }}'" type="button" class="btn btn-primary btn-lg" style="width:15%"><b>LDG/Isochecker</b></button>&nbsp;&nbsp;
