...
$(document).on('click', '.show-vcomments-modal', function() {
    ...
    <tr>
        ...
        $dxf= "../public/storage/isoctrl/iso/attach/".$afilename[0].".dxf";
           ...
           $issued = DB::select("SELECT * FROM hisoctrls WHERE id=(SELECT max(id) FROM hisoctrls WHERE  filename='".$filename[$i]."')");
      ...
      <td>
        ...
        <?php if (auth()->user()->hasRole('IsoctrlAdmin')){ ?> 
          ...
          <?php if ($issued[0]->issued==1){ ?>
            ...
            <a class="comments-iso-modal btn btn-xs btn-success" data-filename ="<?php echo $filename[$i]; ?>" data-toggle="modal" data-target="#commentsfromisoModal">To Issue</a>
            <a class="upload-design-modal btn btn-xs btn-info" data-filename ="<?php echo $filename[$i]; ?>" data-pathfrom="iso" data-toggle="modal" data-target="#uploadfromdesignModal">Upload File</a>
    ...
    <center>
        ...
        <img src="{{ asset('images/arrow-icon.png') }}" style="width:30px" >
        <button onclick="location.href='{{ url('iso') }}'" type="button" class="btn btn-primary btn-lg">IsoController</button>
    </center>
