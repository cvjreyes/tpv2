...
// SHOW/UPDATE MILESTONES elec 
$(document).on('click', '.edit-milestoneselec-modal', function() {
...
@role('elecAdmin')
<table border id="milestoneselec" class="table table-hover table-condensed" style="width: 100%;font-size: 14px;font-weight: normal;white-space: nowrap">
    <center><thead>
        <tr>
            <th>Area</th>
            <th>Week</th>
...
$(document).ready(function() {
    oTable = $('#milestoneselec').DataTable({
        
        "processing": true,
        "serverSide": false,
        "ajax": "{{ route('electrical.milestones') }}",
        "columns": [
            {data: 'area', name: 'area'},
            {data: 'week', name: 'week'},
...
@endrole
  @extends('electrical.editmilestoneselec')
...
<script type="text/javascript">
  ...
  <center>
  <button onclick="location.href='{{ url('elecs') }}'" type="button" class="btn btn-primary btn-lg">Estimated</button>
