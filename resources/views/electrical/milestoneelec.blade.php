    ...
    $(function () {
        $('.add').click(function () {
            ...
            var tr = '<tr class="fila">' +
                    ...
                    '<td>{!! Form::select('units_id[]', [null => 'Select Area...'] + $units, null, array( 'style'=>'height:31px','relecred')) !!}</td>'+
                    '<td>{!! Form::number('est_qty[]', null, array('placeholder' => 'Quantity','class' => 'form-control','relecred')) !!}</td>'+
...
<table id="milestoneelec" class="table table-hover table-condensed" style="width:100%">
    <center><thead>
        <tr>
            <th>Date</th>
            <th>Area</th>
...
$(document).ready(function() {
    oTable = $('#milestoneelec').DataTable({
        ...
        "processing": true,
        "serverSide": false,
        "ajax": "{{ route('electrical.milestone') }}",
        "columns": [
            {data: 'date', name: 'date'},
            {data: 'area', name: 'area'},
...
@extends('electrical.createmilestoneelec')
<br>
<br>
<br>
...
<br>
  ...
  <button onclick="location.href='{{ url('glineelec') }}'" type="button" class="btn btn-primary btn-lg">LineChart</button>
